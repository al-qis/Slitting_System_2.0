...
// ── Mother coil ──────────────────────────────────────────────
$stmt = $conn->prepare("SELECT * FROM mother_coil WHERE id = ?");
...
$stmt->bind_param("i", $mother_id);
$stmt->execute();
$slit_res = $stmt->get_result();
$stmt->close();
$slit_rows = [];
while ($r = $slit_res->fetch_assoc()) $slit_rows[] = $r;

// ── 2. Recoiling entries — use slitting_product_id FK ──
$recoiling_map = [];
if (!empty($slit_rows)) {
...
$all_slit_ids = array_unique(array_map('intval', $all_slit_ids));

if (!empty($all_slit_ids)) {
    // Build a safe IN clause with placeholders
    $placeholders = implode(',', array_fill(0, count($all_slit_ids), '?'));
    $types = str_repeat('i', count($all_slit_ids));

    $stmt = $conn->prepare("
        SELECT rp.*, rp.slitting_product_id AS source_slit_id
        FROM reslit_product rp
        WHERE rp.slitting_product_id IN ($placeholders)
        ORDER BY rp.id ASC
    ");
    $stmt->bind_param($types, ...$all_slit_ids);
    $stmt->execute();
    $rs_res = $stmt->get_result();
    $stmt->close();
    if ($rs_res) {
    ...
    if (!empty($reslit_product_ids)) {
        $rp_placeholders = implode(',', array_fill(0, count($reslit_product_ids), '?'));
        $rp_types = str_repeat('i', count($reslit_product_ids));
        $stmt = $conn->prepare("
            SELECT rr.*
            FROM reslit_rolls rr
            WHERE rr.parent_id IN ($rp_placeholders)
            ORDER BY rr.parent_id ASC, rr.id ASC
        ");
        $stmt->bind_param($rp_types, ...$reslit_product_ids);
        $stmt->execute();
        $rr_res = $stmt->get_result();
        $stmt->close();
        if ($rr_res) {
...
// ── 5. SFC entries ──────────────────────────────────────────
$sfc_rows = [];
$stmt = $conn->prepare("SELECT * FROM sfc WHERE mother_id = ? ORDER BY sfc_id ASC");
$stmt->bind_param("i", $mother_id);
$stmt->execute();
$sfc_res = $stmt->get_result();
$stmt->close();

if (!$sfc_res || $sfc_res->num_rows === 0) {
    // Fallback to lot_no match
    $stmt = $conn->prepare("SELECT * FROM sfc WHERE coil_no = ? AND lot_no LIKE ? ORDER BY sfc_id ASC");
    $lot_like = $mother['lot_no'] . '%';
    $stmt->bind_param("ss", $mother['coil_no'], $lot_like);
    $stmt->execute();
    $sfc_res = $stmt->get_result();
    $stmt->close();
}
...
foreach ($sfc_rows as $sf) {
    ...
    if ($sfAction === 'RESLIT') {
        ...
        if ($rp) {
            $rpId = intval($rp['id']);
            $stmt = $conn->prepare("
                SELECT rr.*, ? AS sfc_lot, ? AS sfc_coil,
                       'RESLIT' AS sfc_process, ? AS parent_status
                FROM reslit_rolls rr
                WHERE rr.parent_id = ?
                ORDER BY rr.id ASC
            ");
            $stmt->bind_param("sssi", $sfLot, $sfCoil, $rp['status'], $rpId);
            $stmt->execute();
            $rr_res = $stmt->get_result();
            $stmt->close();
            if ($rr_res) {
        ...
        if ($rcp) {
            ...
            $stmt = $conn->prepare("
                ...
                ORDER BY id ASC
            ");
            $stmt->bind_param("i", $rcpId);
            $stmt->execute();
            $sp_res = $stmt->get_result();
            $stmt->close();
            if ($sp_res) {
...
$stmt->bind_param("i", $mother_id);
$stmt->execute();
$log_summary = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();
...
<div class="mini-stats">
    <div class="mini-stat"><div class="n text-secondary"><?= $total_rolls ?></div><div class="l">Total Rolls</div></div>
    <div class="mini-stat"><div class="n text-info"><?= $pending_count ?></div><div class="l">Pending</div></div>
    <div class="mini-stat"><div class="n text-primary"><?= $stock_count ?></div><div class="l">In Stock</div></div>
    <div class="mini-stat"><div class="n text-warning"><?= $waiting_count ?></div><div class="l">Waiting QC</div></div>
    <div class="mini-stat"><div class="n" style="color:#2563eb"><?= $approved_count ?></div><div class="l">Approved</div></div>
    <div class="mini-stat"><div class="n text-success"><?= $delivered_count ?></div><div class="l">Delivered</div></div>
</div>
...
<div class="table-responsive">
    <table class="table table-bordered table-sm align-middle text-center" style="font-size:.82rem;">
        ...
        <tr style="background:#faf5ff; border-left:3px solid #7c3aed;">
            <td class="text-muted"><?= $tableIdx++ ?></td>
            <td style="font-family:monospace; font-weight:700; text-align:left;"><?= $sfRef ?></td>
            <td><?= $sfBadge ?></td>
