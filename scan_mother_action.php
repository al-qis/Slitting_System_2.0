...
// ═══════════════════════════════════════════════════════════════
//  QR PARSING — supports two formats, each with an OLD (2-field)
//  and NEW (4-field) shape. WIDTH/LENGTH are optional extras: if
//  missing (old stickers already printed, or plain mother-coil QRs
//  that never carried them), they simply come back as '' — nothing
//  downstream breaks or throws on that. Note: this file already
//  looks up the mother coil's real width/length from mother_coil
//  itself further down, so these QR-scanned values aren't currently
//  used for anything — they're captured here purely so a QR that
//  happens to carry them (e.g. generated by generate_qr.php with
//  ?width=&length=) parses cleanly instead of corrupting coil_no.
//
//  Format A  (KEY=value, all scan sources):
//    Old:  LOT=826277;COIL=FK-1
//    New:  LOT=826277;COIL=FK-1;WIDTH=1250;LENGTH=707
//    Keys matched case-insensitively; segments without "=" skipped.
//
//  Format B  (space-separated, manual overlay entry):
//    Old:  826277 FK-1
//    New:  826277 FK-1 1250 707
// ═══════════════════════════════════════════════════════════════
...
if (strpos($qr, '=') !== false) {
    ...
    // IMPORTANT: limit bumped from 2 to 4 — with limit=2, any extra
    // tokens (width/length) would get crammed into coil_no as one
    // string instead of being split out, corrupting the lookup below.
    $tokens    = preg_split('/\s+/', $qr, 4);
    $lot_no    = trim($tokens[0] ?? '');
    $coil_no   = trim($tokens[1] ?? '');
    $qr_width  = trim($tokens[2] ?? ''); // '' when only 2 tokens present
