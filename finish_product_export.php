...
function cardUrl(string $filterVal, int $month, int $year, string $search, int $day = 0): string {
...
// so they are cumulative and intentionally ignore the Month/Year/Day filter.
// DELIVER is a completed, dated transaction, so it stays scoped to the
// selected month (and day, when one is picked).
...
$stmtDeliver = $conn->prepare("SELECT IFNULL(COUNT(*),0) AS total FROM slitting_product WHERE is_voided=0 AND status='DELIVERED' AND MONTH(delivered_at)=? AND YEAR(delivered_at)=?" . ($day > 0 ? " AND DAY(delivered_at)=?" : ""));
if ($day > 0) {
    $stmtDeliver->bind_param("iii", $month, $year, $day);
} else {
    $stmtDeliver->bind_param("ii", $month, $year);
}
$stmtDeliver->execute();
$deliver = (int)$stmtDeliver->get_result()->fetch_assoc()['total'];
$stmtDeliver->close();
...
$stmtPM = $conn->prepare("SELECT IFNULL(COUNT(*),0) AS total FROM slitting_product WHERE is_voided=0 AND MONTH(date_in)=? AND YEAR(date_in)=?" . ($day > 0 ? " AND DAY(date_in)=?" : ""));
if ($day > 0) {
    $stmtPM->bind_param("iii", $month, $year, $day);
} else {
...
// Same day-aware snapshot instant as the main query's $eom above: end of
// the selected day when one is picked, otherwise end of the whole month.
$eomForCount = ($day > 0)
    ? sprintf('%04d-%02d-%02d 23:59:59', $year, $month, $day)
    : date('Y-m-t 23:59:59', strtotime("$year-$month-01"));
...
<div class="row mb-3 g-2 align-items-center">
    ...
    <div class="col-md-4">
        ...
        <div class="form-text ps-1">Tip: type Lot, Coil, and Roll together separated by spaces (e.g. <code>826613 QA-1 R6</code>) to find an exact roll.</div>
    </div>
    <div class="col-md-5">
...
<div class="d-flex mb-3 gap-2 flex-wrap">
    ...
    <a href="<?= cardUrl('in_pending', $month, $year, $search, $day) ?>"
    ...
    <a href="<?= cardUrl('stock', $month, $year, $search, $day) ?>"
    ...
    <a href="<?= cardUrl('palletised', $month, $year, $search, $day) ?>"
    ...
    <a href="<?= cardUrl('waiting', $month, $year, $search, $day) ?>"
    ...
    <a href="<?= cardUrl('deliver', $month, $year, $search, $day) ?>"
...
<div class="d-flex mb-3 gap-2 flex-wrap">
    ...
    <a href="<?= cardUrl('produced_month', $month, $year, $search, $day) ?>"
       ...
       title="Show every roll produced (date in) during the selected month">
        <div class="card text-center h-100" style="background:#fdf4ff; border-color:#d8b4fe;">
            <div class="card-body p-2">
                <h6 class="mb-1" style="color:#7e22ce;"><i class="bi bi-calendar-plus me-1"></i>PRODUCED (<?= $day > 0 ? $day . ' ' : '' ?><?= date("M", mktime(0,0,0,$month,1)) ?>)</h6>
    ...
    <a href="<?= cardUrl('stock_month_end', $month, $year, $search, $day) ?>"
       ...
       title="Reconstructed stock balance as of the end of the selected month">
        <div class="card text-center h-100" style="background:#fff7ed; border-color:#fdba74;">
            <div class="card-body p-2">
                <h6 class="mb-1" style="color:#c2410c;"><i class="bi bi-clock-history me-1"></i>STOCK @ END OF <?= $day > 0 ? $day . ' ' : '' ?><?= strtoupper(date("M", mktime(0,0,0,$month,1))) ?></h6>
...
<div class="alert alert-info py-2 mb-3">
    ...
    <?= match($filter_card) {
        ...
        'produced_month'   => 'Produced during ' . ($day > 0 ? date("j F Y", mktime(0,0,0,$month,$day,$year)) : date("F Y", mktime(0,0,0,$month,1,$year))),
        'stock_month_end'  => 'Reconstructed stock balance as of end of ' . ($day > 0 ? date("j F Y", mktime(0,0,0,$month,$day,$year)) : date("F Y", mktime(0,0,0,$month,1,$year))),
        default            => ''
    } ?>
    </strong>
