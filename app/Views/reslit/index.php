<?php
$page_title = $page_title ?? 'Reslit Product Management';
...
<style>
    ...
    /* â”€â”€ Compact rolls table for reslit output (matches add_slitting.php) â”€â”€ */
    #rollsTableWrap {
        max-height: 45vh;
        overflow-y: auto;
    ...
    /* â”€â”€ SFC toggle â€” visible pill with icon + text, matches add_slitting.php â”€â”€ */
    .sfc-toggle {
        display: inline-flex;
        align-items: center;
        gap: 6px;
...
<!-- â•â•â• ADD PRODUCT TO RESLIT â€” one unified intake bar â•â•â•â•â•â•â•â•â•â•â•
            ...
            <tbody id="reslitTableBody">
                <?php if (!empty($tableRows)): ?>
                    ...
                    <?php foreach ($tableRows as $row): ?>
                        ...
                        <?php
                        $childRolls = $children[(int)$row['id']] ?? [];
                        if (!empty($childRolls)):
                            foreach ($childRolls as $roll):
                ...
                <div class="row g-3 mb-4" id="summaryKpiRow">
                    ...
                    <div class="col-6">
                        <div class="card border-0 shadow-sm text-center p-3"
                            ...
                            <div class="fw-bold small mb-1"><i class="bi bi-scissors me-1"></i>âœ‚ï¸ Cut Into 2</div>
...
<!-- â•â•â• RESLIT MODAL â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• -->
                        ...
                        <div class="card-body bg-light-subtle">
                            ...
                            <div class="mt-3">
                                ...
                                <div class="form-text mt-1" id="leftoverRouteHint">
                                    Unticked: leftover goes to <strong>Finished Product</strong> only. Ticked: leftover goes to <strong>SFC</strong> only â€” never both.
                                </div>
                            </div>
                        </div>
                    </div>
...
// â”€â”€ Add Product to Reslit â€” one unified intake bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// down) â€” it feeds into the exact same single code path. Same pattern
...
function calculateReslitStock() {
    ...
    // generated) in sync with the Slit Quantity â€” the output rolls are
...
function generateForm() {
    ...
    // â”€â”€ Nominal Length default: for Cut Into 2, this should be the
    // Slit Quantity (Length Used) the user just entered â€” NOT the full
    ...
    // â”€â”€ Bulk "Apply to All" toolbar for New Width + Actual Length (same pattern as add_slitting.php) â”€â”€
    let html = `
        ...
        <div id="bulkApplyBar" class="mb-2 d-flex flex-wrap gap-2 align-items-center">
            ...
            <span class="text-muted small ms-auto">${total} roll${total > 1 ? 's' : ''} â€” scroll the table below if needed</span>
        </div>
...
document.getElementById('reslitForm').addEventListener('submit', function(e) {
    ...
    if (cutType === 'cut_into_2') {
        ...
        if (isNaN(stock) || stock < 0) {
            e.preventDefault();
            alert('Slit Quantity cannot exceed the original length â€” check the Remaining Stock value.');
            return;
        }
    }
...
function loadSummary() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            const typeLabel = { normal: 'âœ‚ï¸ Normal', cut_into_2: 'âœ‚ï¸âœ‚ï¸ Cut Into 2' };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // sfc.php / finish_product.php / recoiling.php) â€” it manages its own
